...
class GenerateDailyDigestJob implements ShouldQueue
{
    ...
    public function handle(AssistantPromptingService $promptingService): void
    {
        ...
        try {
            ...
            // Step 2: Create digest EventObject
            $digestObject = $this->createDigestEventObject();

            // Step 3: Create Event with had_summary action
            $event = $this->createSummaryEvent($digestObject, $this->getBlockCount($digestData));
    ...
    private function createSummaryEvent(EventObject $digestObject, int $blockCount): Event
    {
        $event = Event::create([
            'user_id' => $this->user->id,
            'service' => 'flint',
