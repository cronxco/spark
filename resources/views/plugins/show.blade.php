<x-layouts.app>
    <x-slot:title>{{ $pluginClass::getDisplayName() }} Plugin</x-slot:title>

    <div class="flex flex-col gap-6">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-base-content">{{ $pluginClass::getDisplayName() }}</h1>
            ...
            <div class="card-body">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-lg flex items-center justify-center text-2xl bg-primary text-primary-content">
                        <x-icon name="{{ $pluginClass::getIcon() }}" class="w-8 h-8" />
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center gap-4">
                            <span class="badge badge-primary">{{ $pluginClass::getServiceType() }}</span>
                            <span class="badge badge-outline">{{ $pluginClass::getDomain() }}</span>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            @php
                                $count = \App\Models\EventObject::where('type', $key)
                                    ...
                                    ->where(function($query) {
                                        $query->whereHas('actorEvents.integration', function($q) {
                                            $q->where('user_id', auth()->id());
                                        })->orWhereHas('targetEvents.integration', function($q) {
                                            $q->where('user_id', auth()->id());
                                        });
                                    })
                                    ->count();
                                ...
                                $recent = \App\Models\EventObject::where('type', $key)
                                    ...
                                    ->with(['actorEvents.integration', 'targetEvents.integration'])
                                    ->orderBy('created_at', 'desc')
                                    ->limit(5)
                                    ->get();
                        ...
                        @foreach ($pluginClass::getBlockTypes() as $key => $block)
                            @php
                                $count = \App\Models\Block::where(function($query) use ($key) {
                                    ...
                                    ->whereHas('event.integration', function($query) {
                                        $query->where('user_id', auth()->id());
                                    })
                                    ->count();
