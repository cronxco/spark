...
$event->loadMissing(['actor', 'target', 'integration', 'tags']);
$blocksCount = $event->blocks()->count();
@endphp

<div class="card bg-base-100 shadow-xl border border-{{ $domainColor }}/30 overflow-hidden" style="min-width: 300px; max-width: 350px;">
    ...
    <div class="card-body p-4 gap-3">
        ...
        @if ($event->location_address)
            <div class="flex items-start gap-2 p-2 bg-base-200 rounded text-sm">
                <x-icon name="fas.location-dot" class="w-4 h-4 text-{{ $domainColor }} mt-0.5 flex-shrink-0" />
                <span class="text-base-content/80">{{ $event->location_address }}</span>
            </div>
        @endif
